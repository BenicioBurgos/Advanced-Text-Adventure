...
namespace Advanced_Text_Adventure
{
    ...
    {
        public static readonly string[] shades = [" ", "░", "▒", "▓", "█"];
        public static readonly int[] consoleHues = [354, 54, 116, 180, 221, 307];
        public static char? keyDown;
        public static Stack<float>[] inputTimes = [[], [], [], []];
        ...
        public static List<int> noteRanks = [];
        ...
        static void Main(string[] args)
        {
            ...
            //DrawImage("image");
            //DrawImage("cat");
            foreach (Stack<float> stack in inputTimes)
                stack.Push(0);
            timer.Start();
            Console.CursorVisible = false;
            while(true)
            {
                if (noteTimes[noteToSpawn] - 500 < timer.ElapsedMilliseconds)
                {
                ...
                {
                    if (note.time + 100 < timer.ElapsedMilliseconds)
                        removeNotes.Add(note);
                    else
                    {
                        Console.SetCursorPosition(note.lane * 4 + 2, 10 - (int)MathF.Min((note.time - timer.ElapsedMilliseconds) / 50, 10));
                        Console.Write("██");
                    }
                }
                ...
                if (Console.KeyAvailable)
                    keyDown = Console.ReadKey(true).KeyChar;
                else
                    keyDown = null;
                switch (keyDown)
                {
                    case 'd':
                        inputTimes[0].Push(timer.ElapsedMilliseconds);
                        break;
                    case 'f':
                        inputTimes[1].Push(timer.ElapsedMilliseconds);
                        break;
                    case 'j':
                        inputTimes[2].Push(timer.ElapsedMilliseconds);
                        break;
                    case 'k':
                        inputTimes[3].Push(timer.ElapsedMilliseconds);
                        break;
                }
                foreach (Stack<float> stack in inputTimes)
                {
                    string c = shades[4 - (int)MathF.Min((timer.ElapsedMilliseconds - stack.Peek()) / 100f, 3)];
                    Console.Write("  " + c + c);
                    //Console.Write(3 - MathF.Min((timer.ElapsedMilliseconds - stack.Peek()) / 100f, 3));
                }
                //Console.Write("                                     \r\n");
