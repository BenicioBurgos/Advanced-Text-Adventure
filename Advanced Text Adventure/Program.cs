...
namespace Advanced_Text_Adventure
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while(true)
            {
                if (noteToSpawn < noteTimes.Count && noteTimes[noteToSpawn] - (50 / scrollSpeed) * height < timer.ElapsedMilliseconds)
                {
                ...
                foreach (Note note in notes)
                {
                    ...
                    //Console.Write("  ");
                    //note.oldPos = height - (int)MathF.Min((note.time - timer.ElapsedMilliseconds) / (50 / scrollSpeed), height);
                    //Console.SetCursorPosition(note.lane * 4 + 2, note.oldPos);
                    //Console.Write("██");
                    Vector2 pos = new(note.lane * 4 + 2, note.oldPos);
                    if (note.time + 150 < timer.ElapsedMilliseconds)
                    ...
                    else if (inputsDown[note.lane])
                    {
                        ...
                        if (first && MathF.Abs(note.time - timer.ElapsedMilliseconds) <= 400)
                            removeNotes.Add(note);
                    }
                    else
                    {
                    ...
                    //if (note.holdTime > 0)
                    //{
                    //    int holdEnd = height - (int)MathF.Min((note.time + note.holdTime - timer.ElapsedMilliseconds) / (50 / scrollSpeed), height);
                    //    int curAt = note.oldPos;
                    //    while (curAt > 0)
                    //    {
                    //        curAt--;
                    //        Console.SetCursorPosition(note.lane * 4 + 2, curAt);
                    //        if (curAt >= holdEnd)
                    //            Console.Write("▒▒");
                    //        else
                    //            Console.Write("  ");
    ...
    class Note (int time, int lane, int holdTime, int oldPos = 0)
    {
